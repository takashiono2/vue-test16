    ...
    <table class="Lists">
      ...
      <tbody>
        <tr v-for="(item, index) in display_todos" :key="index">
          ...
          <td><button class="button button--delete" @click="remove(item)">削除</button></td><!--remove削除したら-->
        </tr>
      </tbody>
    </table>
  </section>
...
export default {
  ...
  methods: {
    insert(){
      if(this.content != ''){//insertボタン押した時 (this.content != ''）だった時、insert関数を実行し、contentオブジェクトを送る
